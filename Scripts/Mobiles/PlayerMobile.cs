	...
	public partial class PlayerMobile : Mobile, IHonorTarget
	{
		...
		*/
        ...
        public int GetInsuranceCost(Item item)
        {
            ...
            double multiplier = 1.0; //cost multiplier

            if (item.GetType().IsAssignableFrom(typeof(Factions.FactionItem)))
                cost = (int)(800 * multiplier);
            else if (imbueWeight > 0)
                cost = (int)(Math.Min(800, Math.Max(10, imbueWeight) * multiplier));
            else if (Mobiles.GenericBuyInfo.BuyPrices.ContainsKey(item.GetType()))
                cost = (int)(Math.Min(800, Math.Max(10, Mobiles.GenericBuyInfo.BuyPrices[item.GetType()]) * multiplier));
            else if (item.LootType == LootType.Newbied)
                cost = (int)(10 * multiplier);
